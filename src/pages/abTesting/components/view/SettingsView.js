    ...
    <OGOnlyWrapper as={Panel.LEGACY}>
        ...
        <LabelledValue label="Test Mode">
          ...
          {test.test_mode === 'bayesian' && (
            <p className={styles.HelpText}>
              In bayesian mode, once this test has completed, the best performing template will be
              used in subsequent transmissions in place of the default.
            </p>
          )}
          {test.test_mode === 'learning' && (
            <p className={styles.HelpText}>
              In learning mode, once this test has completed, subsequent transmissions will revert
              to using the default template
            </p>
          )}
        </LabelledValue>
        {test.confidence_level && test.test_mode === 'bayesian' && (
          <LabelledValue label="Confidence Level">
      ...
      <Panel.LEGACY.Section>
        ...
        {test.engagement_timeout && (
          <LabelledValue label="Engagement Timeout">
            <p>{`${test.engagement_timeout.toLocaleString()} hours`} </p>
          </LabelledValue>
        )}
        <LabelledValue label="Distribution Method">
          <p>{snakeToFriendly(test.audience_selection)}</p>
        </LabelledValue>
        {test.total_sample_size && (
          <LabelledValue label="Total Sample Size">
